var api = "6924098a07894abc8dfc5bd00dea249e";
...
async function handleFormSubmit(event) {
  ...
  if (userId) {
    try {
      const response = await axios.put(`https://crudcrud.com/api/${api}/rating/${userId}`, myObj);
    ...
    try {
      const response = await axios.post(`https://crudcrud.com/api/${api}/rating`, myObj);
      console.log(response);
      domContentLoader();
      event.target.reset();
    } catch (err) {
...
async function domContentLoader() {
  ...
  try {
    const response = await axios.get(`https://crudcrud.com/api/${api}/rating`);
...
async function deleteUser(id) { 
  try {
    const response = await axios.delete(`https://crudcrud.com/api/${api}/rating/${id}`);
